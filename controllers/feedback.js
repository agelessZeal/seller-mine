    ...
    Feedback.findOne({ feedback_id: feedback.id }, (err, existingItem) => {
      ...
      if (existingItem) {
        ...
        let feedbackData = {
          ...
          , link: feedback.link, rating: feedback.rating, comment: feedback.text, author: feedback.author, feedback_id:feedback.id
        };
            ...
            $group: {
              ...
              "total": { "$sum": 1},
                ...
                Feedback.find({
                    ...
                    return res.status(200).json({ totalFeedbackRequestRemoval: totalRemoveRequest.length, totalNegativeFeedback: totalNegativeFeedback.length, totalNeutralFeedback: totalNeutralFeedback.length, feedbacks: feedbackData });
                });
              ...
              Feedback.find({
                  ...
                  if ((date_start && date_end) && (filter_items && filter_items.length)) {
                    ...
                    Feedback.find(
                      ...
                      const feedbackData = feedback.map(function(fd) {
                        return {
                          feedback: { id: fd.id, remove_request: fd.remove_request, title: fd.title, details: fd.comment }, rating: fd.rating, author: fd.author,
                    ...
                    Feedback.find({sellerId: sellerId}).limit(150).exec(function(err, feedback) {
                      ...
                      const feedbackData = feedback.map(function(fd) {
                        return {
                          feedback: { id: fd.title, remove_request: fd.remove_request, title: fd.title, details: fd.comment }, rating: fd.rating, author: fd.author,
                          product_info: { id: fd.product_id, title: fd.product_title }, date: fd.date
                        };
                      });
                      return res.status(200).json({ totalFeedbackRequestRemoval: totalRemoveRequest.length, totalNegativeFeedback: totalNegativeFeedback.length, totalNeutralFeedback: totalNeutralFeedback.length, feedbacks: feedbackData });
